...
def run_once(dry_run: bool, branch: str | None = None, create_branch: bool = False) -> int:
    ...
    If branch differs from current, switches to it, commits+pushes, then switches back.
    ...
    if target_branch != origin_branch:
        if dry_run:
            log.info("[DRY-RUN] Would switch back to %s", origin_branch)
        else:
            log.info("Switching back to %s", origin_branch)
            git(["checkout", origin_branch], check=False)
